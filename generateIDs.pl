...
$files[0] = "";
for (my $i =1; $i <= 22; $i++){
	$files[$i] = "/export/home/yusuf/projects/samples/1000genomes/ALL.chr$i.phase3_shapeit2_mvncall_integrated_v5.20130502.genotypes.vcf.gz";
}
$fileX = "/export/home/yusuf/projects/samples/1000genomes/ALL.chrX.phase3_shapeit2_mvncall_integrated.20130502.genotypes.vcf.gz";

# not added
my $file; my $type; my $noise; my $sex;
my $prefix; my $ucn; my $ref; my $baq;
my $ref_hg; my $sampleName;
my $readCol = 1; my $maq;
my $minRead_depth;
...
sub selection{
	foreach my $sm (@samples){
		my $genID = generate_id('type'=>'tbi', 'file'=> "asds", 'hg'=>'hg19', 'sex'=> 1,'sampleName'=>$sm);
		print "$sm\t$genID\n";
	}
}
...
sub bulk{
	...
	my $dbh = DBI->connect($dsn, "test", "something");
	my $sth = $dbh->prepare("INSERT INTO samples(id,samplename) VALUES (?,?) ON DUPLICATE KEY UPDATE id=id");

	foreach my $sample (@h){
		my $genID = generate_id('type'=>'tbi', 'file'=> "asds", 'hg'=>'hg19', 'sex'=> 1, 'sampleName'=> $sample , 'ref'=> 1);
		$sth->execute($genID,$sample);
	}
	$dbh->disconnect();
}
