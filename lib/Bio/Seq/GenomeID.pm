...
=comment
	THIS SECTION OF THE CODE IS FOR 
...
=head1 SYNOPSIS
  ...
  use Bio::Seq::GenomeID wq('generate_id');
  generate_id( <input args> )
...
Where OBJ is of %HASH, will return a genomeID for that particular input

eg: generate_id( file => 'myBam.bam', type => 'bam', hg => 'hg19' )

will return a genomeID from the sample's bam file under the reference genome 'hg19'

%hash = (
	file 	=> [Str] <required>
		# path to sample file (tbi, vcf, or bam types are currently supported)

	hg 	=>	[Str]	<optional> 
		# version of the human genome (eg: hg38 )

	type 	=> [Str] <required>
		# type of input file ( tbi | vcf | bam )
	
	baq 	=> [Int default: 30] <optional>
		# 

	noise	=> [Flt default: 0.05] <optional>
		# threshold for maximum allowed noise

	sex 	=> [Bool default: 0] <optional>
		# 1 to generate sex chromosome marker bits

	ucn	=> [Bool default: 0] <optional>
		# 1 if sex chromosome has an unusual copy number

	ref 	=> [Bool default: 0] <optional>
		# if set to 1, matched to the reference genome if marker is not found

	sampleName => [Str] <optional>
		# specify specifig sample if multiple samples are in file
			)
